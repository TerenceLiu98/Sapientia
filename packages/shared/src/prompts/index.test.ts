...
describe("loadPrompt", () => {
	...
	it("returns the paper-compile-v1 template", () => {
		...
		expect(template).toContain("Sapientia wants reading atoms, not a broad scientific keyphrase list")
	...
	it("returns the paper-compile-reduce-v1 template", () => {
		...
		expect(template).toContain("Keep only candidates that are core or supporting at the whole-paper level")
