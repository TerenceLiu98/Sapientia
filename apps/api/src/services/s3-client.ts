...
function createS3Client(endpoint: string) {
	...
	// Belt-and-suspenders: even with
	// `responseChecksumValidation: "WHEN_REQUIRED"` the SDK's
	// flexible-checksums middleware still injects
	// `x-amz-checksum-mode=ENABLED` into the request, which becomes part of
	// the SigV4 canonical query string. MinIO doesn't recognise it but still
	// folds it into its own signature calculation → mismatch → 403. We strip
	// both the header and the query param from every request before it reaches
	// the signer. AWS's real S3 ignores this strip; MinIO is happy without it.
...
// Internal client for server-side reads/writes. This can point at a cluster
// service name such as `http://minio:9000`.
export const s3Client = createS3Client(config.S3_ENDPOINT)

// Separate client for presigning browser-facing URLs. The host baked into a
// presigned URL is part of the signature, so we cannot sign against the
// internal service name and then rewrite it later.
const s3PresignClient = createS3Client(config.S3_PUBLIC_ENDPOINT)
